(function(Player) {
    ...
    function _sync() {
        player = Player.state;
        // v9p33river367: legacy/inherited saves may omit inventory, but the
        // vitals ticks read food/drink counts directly.
    ...
    function tickHunger() {
        ...
        if (player.townId && !player.traveling) {
            ...
            var _townMarket = town && town.market;
            var _townSupply = _townMarket && _townMarket.supply;
            var _townPrices = (_townMarket && _townMarket.prices) || {};
            ...
            // v9p33river367: skip market auto-buy in towns/outposts that lack
            // a full market supply map.
            if (town && _townSupply && player.hunger < 80) {
                ...
                while (player.hunger < 80) {
                    ...
                    for (var _ci = 0; _ci < foodTypes.length; _ci++) {
                        ...
                        var _sup = _townSupply[_fid] || 0;
                        if (_sup <= 0) continue;
                        var _pr = _townPrices[_fid] || 5;
                    ...
                    if (_cheapestId) {
                        var _staleM = _isNextFoodStale(_townMarket, _cheapestId);
                        _townSupply[_cheapestId]--;
                        Engine.removeFoodCohort(_townMarket, _cheapestId, 1);
        ...
        if (player.hunger <= 0) {
            ...
            if (!player._lastStarveTick || Engine.getDay() > player._lastStarveTick) {
                ...
                if (player.health <= 0 && player.alive && !window._godInvincible) {
                    ...
                    EventTypes.emit('PLAYER_DIED_STARVATION', {
            ...
            if (!player._lastLowVitalHealthTick || Engine.getDay() > player._lastLowVitalHealthTick) {
                ...
                if (vitalDrain > 0) {
                    ...
                    if (player.health <= 0 && player.alive && !window._godInvincible) {
                        ...
                        EventTypes.emit('PLAYER_DIED_VITAL_CAUSE', {
                            playerFullName: player.fullName,
                            cause: cause,
                            deathCause: cause
                        });
    ...
    function checkEnergyForAction(energyCost) {
        ...
        if (energy <= ENERGY_CONFIG.COLLAPSE_THRESHOLD) {
            ...
            if (rng && rng.chance(ENERGY_CONFIG.COLLAPSE_CHANCE)) {
                ...
                EventTypes.emit('PLAYER_COLLAPSED_EXHAUSTION', {
                    playerFullName: player.fullName
                });
        ...
        if (player.traveling && !_onAutoMission) {
            if ((player.energy || 0) < 10 && !player.resting) {
                if (!player._campPromptNeeded) {
                    ...
                    EventTypes.emit('PLAYER_EXHAUSTED_WARNING');
            ...
            if (best) {
                ...
                if (result && result.success) {
                    EventTypes.emit('PLAYER_AUTO_REST', {
                        restName: best.name,
                        cost: best.cost || 0,
                        _noToast: true
                    });
    ...
    function restForTicks(locationId, ticks) {
        ...
        EventTypes.emit('PLAYER_RESTED_ENERGY', {
            playerFullName: player.fullName,
            restLabel: restLabel,
    ...
    function tickThirst() {
        ...
        if (player.townId && !player.traveling) {
            ...
            if (!player.militaryActive) {
            var town = Engine.findTown(player.townId);
            var _bevMarket = town && town.market;
            var _bevSupply = _bevMarket && _bevMarket.supply;
            var _bevPrices = (_bevMarket && _bevMarket.prices) || {};
            // v9p33river367: some towns/outposts lack market supply maps; skip
            // auto-buy instead of crashing the thirst tick.
            if (town && _bevSupply && player.thirst < 60) {
                ...
                while (player.thirst < 80) {
                    ...
                    for (var bi = 0; bi < THIRST_CONFIG.BEVERAGE_TYPES.length; bi++) {
                        ...
                        var _bSupply = (_bevSupply[_bevId] || 0);
                        if (_bSupply <= 0) continue;
                        var _bPrice = _bevPrices[_bevId] || 2;
            ...
            if (player.thirst <= 0) {
                ...
                if (!player._lastDehydrateTick || Engine.getDay() > player._lastDehydrateTick) {
                    ...
                    if (player.health <= 0 && player.alive && !window._godInvincible) {
                        ...
                        EventTypes.emit('PLAYER_DIED_DEHYDRATION', {
                            playerFullName: player.fullName,
