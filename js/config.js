...
const SPOUSE_QUIRKS = [
    // ---- POSITIVE QUIRKS (27) ----
    { id: 'animal_lover', name: 'Animal Lover', icon: '🐾', positive: true, effect: 'Livestock businesses +15% productivity', heirEffect: '+1 Trading SP' },
    { id: 'green_thumb', name: 'Green Thumb', icon: '🌿', positive: true, effect: 'Farm buildings +20% output', heirEffect: 'Farming knowledge' },
    { id: 'merchant_family', name: 'Merchant Family', icon: '💰', positive: true, effect: '5% better trade prices', heirEffect: '+2 starting SP' },
    { id: 'noble_blood', name: 'Hidden Noble Blood', icon: '👑', positive: true, effect: 'Easier rank advancement', heirEffect: 'Starts one rank higher' },
    { id: 'healer', name: 'Healer', icon: '🌡️', positive: true, effect: 'Reduces illness risk for family', heirEffect: 'Herbalism knowledge' },
    { id: 'bookworm', name: 'Bookworm', icon: '📚', positive: true, effect: 'Player XP gain +10%', heirEffect: '+3 bonus SP' },
    { id: 'adventurous', name: 'Adventurous Spirit', icon: '🧭', positive: true, effect: 'Travel time -10%', heirEffect: 'Navigation knowledge' },
    { id: 'forgiving', name: 'Forgiving Nature', icon: '🕊️', positive: true, effect: 'Relationship recovers +0.3/day faster after drops', heirEffect: 'Better starting relationships' },
    { id: 'natural_leader', name: 'Natural Leader', icon: '⭐', positive: true, effect: 'Workers +10% productivity', heirEffect: '+2 Leadership SP' },
    { id: 'good_cook', name: 'Good Cook', icon: '🍲', positive: true, effect: 'Hunger drains 15% slower', heirEffect: 'No hunger penalty first 30 days' },
    { id: 'charming_smile', name: 'Charming Smile', icon: '😊', positive: true, effect: 'NPC relationship gains +10%', heirEffect: '+1 Social SP' },
    { id: 'sailors_daughter', name: "Sailor's Daughter", icon: '⛵', positive: true, effect: 'Ship travel 15% faster/safer', heirEffect: 'Seafaring knowledge' },
    { id: 'blacksmiths_kin', name: "Blacksmith's Kin", icon: '🔨', positive: true, effect: 'Weapon/armor buildings +15%', heirEffect: 'Crafting knowledge' },
    { id: 'silver_tongue', name: 'Silver Tongue', icon: '🪙', positive: true, effect: '2% better negotiation prices', heirEffect: '+1 Trading SP' },
    { id: 'thrifty', name: 'Thrifty', icon: '🧵', positive: true, effect: 'Building maintenance costs -10%', heirEffect: 'Starts with 10% more gold' },
    { id: 'lucky', name: 'Lucky', icon: '🍀', positive: true, effect: '5% chance to avoid bad random events', heirEffect: 'Inherits luck trait' },
    { id: 'musical', name: 'Musical', icon: '🎵', positive: true, effect: 'Festival bonuses +20%, worker morale +5%', heirEffect: '+1 Social SP' },
    { id: 'keen_eye_quirk', name: 'Keen Eye', icon: '👁️', positive: true, effect: 'Spots counterfeit goods, market insight', heirEffect: 'Appraisal knowledge' },
    { id: 'patient', name: 'Patient', icon: '🧘', positive: true, effect: 'Production quality +5%', heirEffect: '+1 Crafting SP' },
    { id: 'diplomatic', name: 'Diplomatic', icon: '🤝', positive: true, effect: 'Kingdom reputation gains +10%', heirEffect: 'Starts with +10 reputation' },
    { id: 'strong_constitution', name: 'Strong Constitution', icon: '💪', positive: true, effect: '30% less likely to get sick', heirEffect: 'Strong health' },
    { id: 'early_riser', name: 'Early Riser', icon: '🌅', positive: true, effect: 'Morning productivity bonus (first 6 hours)', heirEffect: 'Discipline trait' },
    { id: 'protective', name: 'Protective', icon: '🛡️', positive: true, effect: 'Family 25% less likely to be attacked/robbed', heirEffect: 'Combat awareness' },
    { id: 'generous_spirit', name: 'Generous Spirit', icon: '💝', positive: true, effect: '+5% reputation gains, NPCs more helpful', heirEffect: 'Goodwill from NPCs' },
    { id: 'quick_learner', name: 'Quick Learner', icon: '🎓', positive: true, effect: 'Heir gets +2 SP on inheritance', heirEffect: '+2 SP on regency' },
    { id: 'well_connected', name: 'Well Connected', icon: '🔗', positive: true, effect: 'Market intel from other towns', heirEffect: 'Trade connections' },
    { id: 'loyal_heart', name: 'Loyal Heart', icon: '❤️', positive: true, effect: '+20 regency score bonus. Never abandons', heirEffect: 'Loyalty trait' },
    { id: 'fertile', name: 'Fertile', icon: '🌸', positive: true, effect: '50% higher chance of children', heirEffect: 'May inherit fertility' },
    // ---- NEGATIVE QUIRKS (28) ----
    { id: 'jealous', name: 'Jealous', icon: '😤', positive: false, effect: 'Relationship decays 2x when away from spouse', heirEffect: 'No effect' },
    { id: 'secret_gambler', name: 'Secret Gambler', icon: '🎲', positive: false, effect: 'Loses 10-100g randomly every 30-60 days', heirEffect: 'May inherit habit (-1 SP)' },
    { id: 'violent_temper', name: 'Violent Temper', icon: '💢', positive: false, effect: 'Random arguments drop relationship -5 to -15 every 30-60 days', heirEffect: 'May be fearful (-1 Social SP)' },
    { id: 'frail_health', name: 'Frail Health', icon: '🤒', positive: false, effect: '2x chance of spouse death', heirEffect: 'May inherit weak health' },
    { id: 'gossip', name: 'Gossip', icon: '🗣️', positive: false, effect: 'May reveal your crimes to authorities (+10% detection)', heirEffect: 'No effect' },
    { id: 'stubborn', name: 'Stubborn', icon: '🪨', positive: false, effect: 'Relationship gains -25%', heirEffect: 'Willpower (+1 SP, mixed)' },
    { id: 'spendthrift', name: 'Spendthrift', icon: '💸', positive: false, effect: 'Wastes 10% of monthly income on luxuries', heirEffect: 'Expensive tastes' },
    { id: 'paranoid', name: 'Paranoid', icon: '😰', positive: false, effect: 'Harder to maintain relationship, suspicious of gifts', heirEffect: 'Trust issues (-1 Social SP)' },
    { id: 'lazy', name: 'Lazy', icon: '😴', positive: false, effect: 'Building productivity -10% for spouse-managed buildings', heirEffect: 'May be lazy (-1 SP)' },
    { id: 'vain', name: 'Vain', icon: '💅', positive: false, effect: 'Demands gifts worth 50g+ every 30 days or relationship -10', heirEffect: 'Vanity trait' },
    { id: 'superstitious', name: 'Superstitious', icon: '🌙', positive: false, effect: 'Refuses certain goods, seasonal mood swings', heirEffect: 'No effect' },
    { id: 'clumsy', name: 'Clumsy', icon: '🫗', positive: false, effect: '3% chance per month to damage stored inventory', heirEffect: 'No effect' },
    { id: 'hot_headed', name: 'Hot Headed', icon: '🔥', positive: false, effect: 'May start fights with NPCs, drawing guard attention', heirEffect: 'Temper (-1 Social SP)' },
    { id: 'pessimist', name: 'Pessimist', icon: '😔', positive: false, effect: 'Worker morale -5%, occasional negative commentary', heirEffect: 'Cautious nature (mixed)' },
    { id: 'fearful', name: 'Fearful', icon: '😨', positive: false, effect: "Won't travel with you, panics during war", heirEffect: 'Anxiety trait' },
    { id: 'vengeful', name: 'Vengeful', icon: '⚡', positive: false, effect: 'If relationship drops below 30, actively sabotages business', heirEffect: 'No effect' },
    { id: 'kleptomaniac', name: 'Kleptomaniac', icon: '🫳', positive: false, effect: 'Steals 5-20g per month from your businesses', heirEffect: 'May inherit (-1 Honesty)' },
    { id: 'sickly', name: 'Sickly', icon: '🤢', positive: false, effect: "Frequently ill, can't contribute, may need medicine", heirEffect: 'Weak constitution' },
    { id: 'night_terrors', name: 'Night Terrors', icon: '😱', positive: false, effect: '-5% town reputation from disturbing neighbors', heirEffect: 'No effect' },
    { id: 'drunkard', name: 'Drunkard', icon: '🍺', positive: false, effect: 'Spends 20-50g/month on drink, erratic behavior', heirEffect: 'May inherit habit' },
    { id: 'prideful', name: 'Prideful', icon: '👃', positive: false, effect: "Won't do menial work, demands higher social rank", heirEffect: 'Ambitious (mixed)' },
    { id: 'secretive', name: 'Secretive', icon: '🤫', positive: false, effect: 'Trait reveal takes 2x as many dates', heirEffect: 'Secretive nature' },
    { id: 'clingy', name: 'Clingy', icon: '🫂', positive: false, effect: 'Must be in same town or -0.5/day relationship decay', heirEffect: 'Dependent nature' },
    { id: 'manipulative', name: 'Manipulative', icon: '🎭', positive: false, effect: 'May turn children against you (-relationship with heir)', heirEffect: '-10 starting relationship with NPCs' },
    { id: 'cursed_lineage', name: 'Cursed Lineage', icon: '💀', positive: false, effect: 'Children have 2x illness/death risk', heirEffect: 'Weak constitution' },
    { id: 'criminal_past', name: 'Criminal Past', icon: '🔪', positive: false, effect: 'Knows underworld (-5% detection) BUT may attract criminal attention', heirEffect: '+1 Underworld SP' },
    { id: 'hoarder', name: 'Hoarder', icon: '📦', positive: false, effect: 'Stockpiles 10% of produced goods, reducing sellable inventory', heirEffect: 'No effect' },
    { id: 'low_fertility', name: 'Low Fertility', icon: '🥀', positive: false, effect: 'Conception takes much longer (~100 days avg)', heirEffect: 'May inherit low fertility' },
    { id: 'infertile', name: 'Infertile', icon: '🚫', positive: false, effect: 'Cannot have children', heirEffect: 'No children possible', rare: true },
];
